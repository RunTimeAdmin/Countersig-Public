...
export function registerIdentityTools(): ToolDefinition[] {
  return [
    // ─── register_agent ───────────────────────────────────────────────
    {
      name: 'register_agent',
      ...
      handler: async (args) => {
        try {
          ...
          const chainType = (args.chainType as string) || 'generic';
          ...
          const result = await (client.agents as any).register({
          ...
          const response = {
            ...
            publicKey: pubkey,
            message:
              'Agent registered. Call verify_agent to complete cryptographic verification.',
          };
    ...
    // ─── verify_agent ─────────────────────────────────────────────────
    {
      name: 'verify_agent',
      description:
        "Complete cryptographic verification of the registered agent using PKI challenge-response. Upgrades agent status from 'unverified' to 'verified'. The private key stored during registration is used automatically.",
      inputSchema: {
        type: 'object',
        properties: {
          agentId: {
      ...
      handler: async (args) => {
        try {
          ...
          const challengeRes = await axios.post(
            `${config.apiUrl}/verify/challenge`,
            { agentId },
            { headers }
          );
          ...
          const sig = nacl.sign.detached(challengeBytes, secretKey);
          const signature = bs58.encode(sig);

          // Step 3: Submit response
          const verifyRes = await axios.post(
            `${config.apiUrl}/verify/response`,
            { agentId, nonce, signature },
            { headers }
          );
          ...
          const response = {
            ...
            message:
              'Agent cryptographically verified. Reputation scoring is active.',
          };
    ...
    // ─── get_agent ────────────────────────────────────────────────────
    {
      name: 'get_agent',
      description:
        'Retrieve the current status, reputation score, and metadata for an agent.',
      inputSchema: {
        type: 'object',
        properties: {
          agentId: {
            type: 'string',
            description:
              'Agent ID to look up. Defaults to locally configured agent.',
          },
        },
      },
      handler: async (args) => {
    ...
    // ─── get_reputation ───────────────────────────────────────────────
    {
      name: 'get_reputation',
      description:
        'Get the full reputation breakdown for any agent. Use to evaluate trustworthiness before interacting.',
      inputSchema: {
        type: 'object',
        properties: {
          agentId: {
      ...
      handler: async (args) => {
        try {
          ...
          const client = getClient();
          const result = await (client as any).reputation.get(agentId);
